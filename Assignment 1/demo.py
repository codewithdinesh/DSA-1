""" 
Problem Statement: Consider telephone book database of N clients. Make use of a hash table implementation to quickly look up clients telephone no, make use of two collision habdling techniques and compare then using no. comparinsion are required to find a set of telephone no.
        ...
        # if element is not found at position returned hash function
                ...
                while self.table[position] != None or self.comparisons <= self.size:
                    ...
                    if(self.table[position].get_name() == record.get_name() and self.table[position].get_number() == record.get_number()):
                        ...
                        print("Phone number found at position {} ".format(
                            position) + " and total comparisons are " + str(i))
                        return position
